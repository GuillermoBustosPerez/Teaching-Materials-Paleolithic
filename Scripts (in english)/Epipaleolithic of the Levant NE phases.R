...
#### Heinrich events aproximate chronology ####
H.events <- data.frame(
...
df.Climate  %>% ggplot(aes(age/1000, delta)) +
  ...
  annotate("rect",
           ...
           ymax = Inf,
           fill = "khaki1") +
  geom_label(aes(x = 22, y = -29, label = "Initial:\nKebarian &\nNebekian\n25-19 Ka"),
  ...
  geom_label(aes(x = 17, y = -29, label = "Middle:\nGeometric\nkebarian\n19-15 Ka"),
  ...
  geom_label(aes(x = 9, y = -29, label = "N e o l i t i c"),
             fill = "white", color = 'black',
             size = 3) +
  
  geom_label(aes(x = 27, y = -28, label = "Upper\nPaleolithic"),
             fill = "white", color = 'black',
             size = 3) +
  ...
  annotate("rect",
           ...
           fill = "lightblue",
           alpha = 0.5) +
  geom_label(aes(x = 12.25, y = -45, label = "YD"),
...
ggsave(
  "Images-english/Phases of the Epipaleolthic of the Levant (NE).jpg",
  device = "jpg",
  dpi = 1200,
  width = 32,
  height = 15,
