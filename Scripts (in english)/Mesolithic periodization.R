...
df.Climate  %>% ggplot(aes(age/1000, delta)) +
  ...
  annotate("rect",
           ...
           ymax = Inf,
           fill = "wheat") +
  geom_label(aes(x = 10, y = -29, label = "Early Mesolithic\n(11 - 9 Ka)"),
  ...
  annotate("rect",
           ...
           ymax = Inf,
           fill = "wheat3") +
  geom_label(aes(x = 7.5, y = -29, label = "Late Mesolithic\n(9 - 6 Ka)"),
  ...
  geom_label(aes(x = 13.5, y = -29, label = "Upper Paleolithic"),
  ...
  geom_label(aes(x = 7, y = -46), label = paste0("H o l o c e n e"),
...
ggsave(
  "Images-english/Mesolithic periodization.jpg",
  device = "jpg",
  dpi = 1200,
  width = 32,
  height = 15,
